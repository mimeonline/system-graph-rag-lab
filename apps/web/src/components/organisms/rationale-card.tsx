...
export function RationaleCard({
  ...
  evidenceTitle,
  rationaleMarker,
}: RationaleCardProps): React.JSX.Element {
  const { rationaleIntro, evidenceItems } = splitRationale(coreRationale, rationaleMarker);
    ...
    <section className="order-4 space-y-4 rounded-xl border border-slate-200/70 bg-white/60 p-4 sm:p-5 lg:order-3">
      ...
      {evidenceItems.length > 0 && (
        <div className="space-y-2 rounded-lg border border-slate-200/70 bg-slate-50/80 p-3">
          <p className="text-xs font-semibold uppercase tracking-[0.1em] text-slate-500">{evidenceTitle}</p>
...
function splitRationale(
  coreRationale: string,
  marker: string,
): { rationaleIntro: string; evidenceItems: string[] } {
