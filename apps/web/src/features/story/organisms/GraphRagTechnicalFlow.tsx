...
import {
    ...
    getStoryChapters,
    getStoryPerspectives,
    getPrimaryActionForChapter,
} from "@/features/story/story-flow-model";
import { trackLiteEvent } from "@/lib/analytics-lite";
import { useCallback, useEffect, useMemo, useRef, useState } from "react";

type GraphRagTechnicalFlowProps = {
  locale: "de" | "en";
};

export function GraphRagTechnicalFlow({
  locale,
}: GraphRagTechnicalFlowProps): React.JSX.Element {
  const storyChapters = useMemo(() => getStoryChapters(locale), [locale]);
  const storyPerspectives = useMemo(() => getStoryPerspectives(locale), [locale]);
  const [activeChapterIndex, setActiveChapterIndex] = useState(0);
  const [perspective, setPerspective] = useState<StoryPerspective>("architecture");
  const sectionRef = useRef<HTMLElement>(null);

  const activeChapter = useMemo(() => storyChapters[activeChapterIndex], [activeChapterIndex, storyChapters]);
  const action = useMemo(
    () => getPrimaryActionForChapter(activeChapterIndex, storyChapters.length, locale),
    [activeChapterIndex, locale, storyChapters.length],
  );
  ...
  const handleAdvance = useCallback(() => {
    setActiveChapterIndex((current) => {
      const next = Math.min(current + 1, storyChapters.length - 1);
      if (next !== current) {
        trackLiteEvent("story_chapter_advance", {
          from: current + 1,
          to: next + 1,
          chapter_id: storyChapters[next].id,
          perspective,
        });
      }
      return next;
  ...
  }, [perspective, scrollToTop, storyChapters]);
    ...
    setActiveChapterIndex((current) => {
      ...
      if (prev !== current) {
        trackLiteEvent("story_chapter_back", {
          from: current + 1,
          to: prev + 1,
          chapter_id: storyChapters[prev].id,
          perspective,
        });
      }
      return prev;
  ...
  const handleStepClick = useCallback(
    (index: number) => {
      const next = Math.max(0, Math.min(index, storyChapters.length - 1));
    ...
    [activeChapterIndex, perspective, scrollToTop, storyChapters],
  );
  ...
  useEffect(() => {
    const onKeyDown = (e: KeyboardEvent) => {
      if (e.key === "ArrowRight" && activeChapterIndex < storyChapters.length - 1) {
        handleAdvance();
  ...
  }, [activeChapterIndex, handleAdvance, handleBack, storyChapters.length]);

  return (
    <section ref={sectionRef} className="space-y-4 rounded-2xl border border-slate-300/80 bg-slate-100/60 p-4 sm:p-5">
      <StoryProgress locale={locale} chapters={storyChapters} activeIndex={activeChapterIndex} onStepClick={handleStepClick} />
      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_330px]">
        ...
        <article key={activeChapter.id} className="animate-chapter-enter space-y-4 rounded-xl border border-slate-300/80 bg-white p-4 sm:p-5">
          <header className="space-y-1.5">
            <p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-slate-500">
              {locale === "en"
                ? `Chapter ${activeChapterIndex + 1} of ${storyChapters.length}`
                : `Kapitel ${activeChapterIndex + 1} von ${storyChapters.length}`}
            </p>
          ...
          <section className="space-y-4 text-sm leading-6 text-slate-700">
            <div className="border-l-2 border-sky-400 pl-3">
              <p className="text-[11px] font-semibold uppercase tracking-[0.1em] text-slate-500">
                {locale === "en" ? "What happens in this step" : "Was in diesem Schritt passiert"}
              </p>
              <p className="mt-1">{activeChapter.technicalFlow}</p>
            </div>
            <div className="border-l-2 border-sky-300 pl-3">
              <p className="text-[11px] font-semibold uppercase tracking-[0.1em] text-slate-500">
                {locale === "en" ? "Why this matters" : "Warum das wichtig ist"}
              </p>
              <p className="mt-1">{activeChapter.structuralRelevance}</p>
            </div>
            <div className="border-l-2 border-slate-300 pl-3">
              <p className="text-[11px] font-semibold uppercase tracking-[0.1em] text-slate-500">
                {locale === "en" ? "How to read the visualization" : "So liest du die Visualisierung"}
              </p>
        ...
        <aside className="flex flex-col gap-4 lg:sticky lg:top-24 lg:self-start">
          <StoryPerspectiveSwitch locale={locale} options={storyPerspectives} perspective={perspective} onChange={handlePerspectiveChange} />

          <section className="rounded-xl border border-slate-300/80 bg-white p-3">
            <p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-slate-500">
              {locale === "en" ? "Viewpoint" : "Blickwinkel"}
            </p>
          ...
          <section className="rounded-xl border border-slate-300/80 bg-white p-3">
            <p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-slate-500">
              {locale === "en" ? "Key terms" : "Kernbegriffe"}
            </p>
          ...
          <section className="rounded-xl border border-slate-300/80 bg-white p-3">
            <p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-slate-500">
              {locale === "en" ? "Before -> After" : "Vorher → Nachher"}
            </p>
