      ...
      <main className="flex-1">
        ...
        <section className="mx-auto grid w-full max-w-295 gap-6 px-4 py-8 sm:px-6 sm:py-12 lg:grid-cols-[minmax(0,1.05fr)_minmax(340px,0.7fr)]">
          ...
          <div className="space-y-6">
            ...
            <section className="rounded-3xl border border-slate-200 bg-white p-6">
              ...
              <p className="mt-3 text-sm font-semibold leading-relaxed text-slate-800">{graphType.risk}</p>
            </section>
          </div>
        </section>
          ...
          <div className="glass-panel rounded-3xl p-5 sm:p-6">
            <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
              ...
              <span className="rounded-full border border-sky-200 bg-sky-50 px-3 py-1 text-xs font-bold uppercase tracking-[0.12em] text-sky-700">
                {locale === "en" ? "Simulation" : "Simulation"}
              </span>
            </div>
        ...
        <section className="mx-auto grid w-full max-w-295 gap-6 px-4 pb-12 sm:px-6 lg:grid-cols-[minmax(0,0.95fr)_minmax(340px,0.65fr)]">
          ...
          <div className="rounded-3xl border border-slate-200 bg-slate-950 p-6 text-white">
            ...
            <p className="mt-5 border-t border-white/10 pt-4 text-sm font-semibold leading-relaxed text-slate-300">
              {locale === "en"
        ...
        <section className="mx-auto w-full max-w-295 px-4 pb-12 sm:px-6 sm:pb-16">
          ...
          <div className="grid gap-4 lg:grid-cols-3">
            {graphType.steps.map((step, index) => (
              <article key={step.label} className="rounded-3xl border border-slate-200 bg-white p-6">
  ...
  return (
    <section className="rounded-3xl border border-slate-200 bg-white p-5">
      <div className={`rounded-2xl border p-4 ${accentClassName}`}>
        <p className="text-xs font-bold uppercase tracking-[0.16em] text-slate-600">{subtitle}</p>
        <h3 className="mt-1 text-2xl font-bold text-slate-950">{title}</h3>
      </div>
      <div className="mt-4 space-y-3">
        {boxes.map((box) => (
          <article key={box.title} className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
            <h4 className="text-base font-bold text-slate-950">{box.title}</h4>
            <p className="mt-2 text-sm font-medium leading-relaxed text-slate-700">{box.body}</p>
          </article>
        ))}
      </div>
    </section>
