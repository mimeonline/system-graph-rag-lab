...
export async function generateMetadata({ params }: BlogArticlePageProps): Promise<Metadata> {
  ...
  if (!post) {
    return {
      title: "Artikel nicht gefunden | System GraphRAG Lab",
    };
  }

  const canonical = post.frontmatter.canonicalUrl ?? withCanonical(`/blog/${slug}`);
  return {
...
export default async function BlogArticlePage({ params }: BlogArticlePageProps): Promise<React.JSX.Element> {
  const { slug } = await params;
  const post = await getBlogPostBySlug(slug);
  if (!post) {
    notFound();
