...
4. Next.js Implementierung bleibt verbindlich TypeScript in Web UI und API Layer.
5. Secrets und Keys werden in `local` über `.env.local` oder `.env` geführt und in `public` über Vercel Environment Variables.
6. Architekturartefakte bleiben synchron zwischen ADR, Arc42, Deployment View, API Spec und Retrieval Contract.
...
3. ADR-0003 fixiert Tech Stack auf Next.js `16.1.6` mit TypeScript, Route Handler, Tailwind CSS, shadcn/ui, Atomic Design und minimaler Observability.
4. ADR-0004 fixiert serverless konsistentes Rate Limiting im Public Profil auf Vercel KV.
5. ADR-0005 fixiert Local Dev Topologie mit Next.js lokal, Neo4j Docker und profilabhängigem Rate Limit Store bei identischem Contractverhalten.
...
5. Retrieval Contract gilt identisch in `public` und `local`.
...
6. Local Reproducibility: feste Local Ports, TypeScript Baseline und profilklare Environment Variablen.

## Open Architecture Questions
1. Welche konkrete lokale Neo4j Versionspinning Regel wird verbindlich gesetzt, um Drift zu minimieren.
2. Welche Referenzdatenbasis wird als lokaler Snapshot für deterministische QA Replays gepflegt.
3. Modellfixierung für P0 Antwortgenerierung inklusive Max Token Setting.
4. Welches CI Gate OpenAPI Konsistenzprüfung gegen `docs/spec/api.md` verbindlich absichert.
5. Ob ein verpflichtendes `.env.example` als nicht geheimes Variablen Template im Repo geführt werden soll.
...
4. Vor Dev Übergabe stets prüfen, dass Public Ziel unverändert bleibt und Local Setup deterministisch reproduzierbar beschrieben ist.
5. Vor Dev Übergabe stets prüfen, dass TypeScript Vorgabe und `.env` oder `.env.local` Secret-Regel in allen Architekturartefakten konsistent sind.
