...
| E3-S1 | AC-1/AC-2 | QA-E3-S1-01 | Hauptantwort, Referenzliste (max. 3 Items) und „Knapper P0-Kernnachweis“ sichtbar nach zwei unterschiedlichen Queries | local | x |  | 2026-02-26: `pnpm --dir apps/web exec vitest run src/features/query/view-model.test.ts` (3 tests, Exit Code 0); Manuelle QA: Dev-Server, mind. zwei Fragen (Default + zusätzliche komplexe Frage), jeweils Hauptantwort, Referenzen und Kernnachweis geprüft (Screenshot/Notizen verfügbar). |
| E3-S2 | AC-1 | QA-E3-S2-01 | Loading, Fehler, Leere zeigen unterscheidbaren Status-Text | local,vercel | x |  | 2026-02-26: `pnpm --dir apps/web exec vitest run src/components/organisms/query-panel-status.test.ts` (3 tests, Exit Code 0) – `getStatusHint` liefert klare Texte und QueryPanel leitet sie in Helper-Text weiter (Code Review). |
| E3-S2 | AC-2 | QA-E3-S2-02 | Jeder Status zeigt klar benannte naechste Aktion | local,vercel | x |  | 2026-02-26: `pnpm --dir apps/web exec vitest run src/components/organisms/query-panel-status.test.ts` (3 tests, Exit Code 0) – `nextAction`-Texte pro Status bestätigt, QueryPanel/QueryInput verwenden diese Hinweise (Code Review). |
