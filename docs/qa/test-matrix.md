...
| E1-S6 | AC-1 | QA-E1-S6-01 | Lokaler Neo4j Seed-Reset und Reseed ist reproduzierbar und Runtime-Reads sind danach wieder erfolgreich | local | x |  | 2026-02-26: `pnpm --dir apps/web lint` Pass; `pnpm --dir apps/web test` Pass (23 Pass, 2 Skip); `pnpm --dir apps/web build` Pass; `pnpm --dir apps/web seed:local:reset-reseed` Pass mit `Importierte Nodes: 105`, `Importierte Relationen: 203`, `Read-Check Nodes: 105`, `Read-Check Relationen: 203`; `pnpm --dir apps/web test -- src/features/seed-data/local-seed-reset.test.ts` Pass (25/25) |
