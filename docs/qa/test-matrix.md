...
| E1-S6 | AC-1 | QA-E1-S6-01 | Lokaler Neo4j Seed-Reset und Reseed ist reproduzierbar und Runtime-Reads sind danach wieder erfolgreich | local | x |  | 2026-02-26: `pnpm --dir apps/web seed:local:reset-reseed` Pass mit `Importierte Nodes: 105`, `Importierte Relationen: 203`, `Read-Check Nodes: 105`, `Read-Check Relationen: 203`; `pnpm --dir apps/web lint` Pass; `pnpm --dir apps/web test` Pass (26 Pass, 2 Skip); `pnpm --dir apps/web build` Pass |
| E1-S6 | AC-2 | QA-E1-S6-02 | Security-Recheck fuer local-only Guard, Opt-In, fail-fast und Delete-Scope ist reproduzierbar bestanden | local | x |  | 2026-02-26: `pnpm --dir apps/web exec vitest run src/features/seed-data/local-seed-reset.test.ts` Pass (5 Pass, 1 Skip); Testfaelle verifizieren non-local reject, missing opt-in reject, no delete on guard-fail, delete-scope `WHERE n.id IN $seedNodeIds`; Epic-Ebene bleibt durch offenes Security-Gate formal blockiert |
